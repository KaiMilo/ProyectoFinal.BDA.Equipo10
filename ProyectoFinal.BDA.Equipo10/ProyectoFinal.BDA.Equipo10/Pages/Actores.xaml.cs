...
// Se estable de donde sale el itemsource
        ...
        //Se presionar y se manda a la página Descripcion
        private async void lsvGrupo_ItemTapped(object sender, ItemTappedEventArgs e)
        {
       //SE manda con el item, el actor seleccionado
            Actor item = (Actor)e.Item;
                if (item != null)
                {
                    await Navigation.PushAsync(new Descripcion(item));
                }
